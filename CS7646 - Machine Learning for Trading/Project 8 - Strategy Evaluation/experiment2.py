...
def run_experiment(ticker):
    ...
    rng = pd.date_range(start_dt, end_dt)
    start_dt = rng[0]
    end_dt = rng[-1]
    ...
    print(f'Ex 2 Elapsed: {time_end - time_start}')
...
def generate_plots(ticker, spdf, mpdf, bdf):
    ...
    plt.title(f"{ticker} manual vs Q-learner vs benchmark ($0 - 0%)")
    plt.xlabel("Date")
    plt.ylabel("Portfolio Value")
    plt.legend()
    plt.savefig(f'{ticker}_experiment_2.png')
    plt.clf()
