...
def compute_portvals(  		  	   		   	 		  		  		    	 		 		   		 		  
    ...
    odf = pd.read_csv(orders_file, parse_dates=True, na_values=['nan']) 
    ...
    pdf = get_data(tickers, pd.date_range(start_date, end_date))  	
    # drop SPY if not in tickers
    pdf = pdf[tickers].copy()
...
def test_code():  		  	   		   	 		  		  		    	 		 		   		 		  
    ...
    idx='$SPX'
    spx = get_data([idx], pd.date_range(start_date, end_date))  
    spx = spx[idx]
