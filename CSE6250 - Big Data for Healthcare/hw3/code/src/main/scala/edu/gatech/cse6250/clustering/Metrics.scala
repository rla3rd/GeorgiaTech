...
object Metrics {
  ...
  def purity(clusterAssignmentAndLabel: RDD[(Int, Int)]): Double = {
    ...
    val k = clusterAssignmentAndLabel.map(_._1).distinct.count

    //println("N: " + n + ", K: " + k)

    val max_wk_cj = clusterAssignmentAndLabel.map(
      x => (x, 1)).keyBy(_._1).reduceByKey(
        (x, y) =>
          (x._1, x._2 + y._2)).map(
          x =>
            (x._2._1._1, x._2._2)).keyBy(_._1).reduceByKey(
            (x, y) =>
              (1, Math.max(x._2, y._2)))

    max_wk_cj.map(println)

    val sumMax = max_wk_cj.map(
      x => x._2._2).reduce(
        (x, y) => x + y)
