...
#############################################
            if not source in self.node_degree:
                self.node_degree[source] = 1
            else:
                self.node_degree[source] += 1
            if source >= target and source > self.max_node_id:
                self.max_node_id  = source
            if source < target and target > self.max_node_id:
...
#############################################
            ...
            prob = (1 - damping_factor) / (self.max_node_id + 1)
            new_scores = [prob + item for item in new_scores]
            scores = new_scores * node_weights
